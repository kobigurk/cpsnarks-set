...
pub fn criterion_benchmark(c: &mut Criterion) {
    ...
    let crs = cpsnarks_set::protocols::membership::Protocol::<ClassGroup, G1Projective, HPProtocol<Bls12_381>>::setup(&params, &mut rng1, &mut rng2).unwrap().crs;
    ...
    c.bench_function("membership_class protocol", |b| b.iter(|| {
        let proof_transcript = RefCell::new(Transcript::new(b"membership"));
        let mut verifier_channel = TranscriptVerifierChannel::new(&crs, &proof_transcript);
        let statement = Statement {
            c_e_q: commitment.clone(),
