...
pub fn criterion_benchmark(c: &mut Criterion) {
    ...
    let crs = cpsnarks_set::protocols::nonmembership::Protocol::<Rsa2048, G1Projective, HPProtocol<Bls12_381>>::setup(&params, &mut rng1, &mut rng2).unwrap().crs;
    let protocol = Protocol::<Rsa2048, G1Projective, HPProtocol<Bls12_381>>::from_crs(&crs);

    let mut value = None;
    for i in 1..1000 {
        let test_value = Integer::from(Integer::u_pow_u(
                2,
                (crs.parameters.hash_to_prime_bits)
                    as u32,
            )) - &Integer::from(i);
        if test_value.is_probably_prime(params.security_level as u32/2) == rug::integer::IsPrime::Probably {
    ...
    c.bench_function("nonmembership_prime_60 protocol", |be| be.iter(|| {
        let proof_transcript = RefCell::new(Transcript::new(b"nonmembership"));
        let mut verifier_channel = TranscriptVerifierChannel::new(&crs, &proof_transcript);
        let statement = Statement {
            c_e_q: commitment.clone(),
