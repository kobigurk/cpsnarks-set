...
use cpsnarks_set::protocols::root::{Protocol, Statement, Witness};
...
pub fn criterion_benchmark(c: &mut Criterion) {
    ...
    let crs = cpsnarks_set::protocols::membership_prime::Protocol::<Rsa2048, JubJubProjective>::setup(&params, &mut rng1, &mut rng2).crs.crs_root;
    ...
    c.bench_function("root protocol", |b| b.iter(|| {
        ...
        protocol.prove(&mut proof_transcript, &mut rng1, &statement, &Witness {
            e: value.clone(),
            r: randomness.clone(),
            w: w.clone(),
        }).unwrap();
