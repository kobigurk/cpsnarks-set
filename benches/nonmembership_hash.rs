...
use cpsnarks_set::{
    ...
    protocols::{
        ...
        hash_to_prime::snark_hash::{Protocol as HPProtocol, HashToPrimeHashParameters},
    },
};
...
pub fn criterion_benchmark(c: &mut Criterion) {
    ...
    let crs = cpsnarks_set::protocols::nonmembership::Protocol::<Rsa2048, G1Projective, HPProtocol<Bls12_381, TestHashToPrimeParameters>>::setup(&params, &mut rng1, &mut rng2).unwrap().crs;
    let protocol = Protocol::<Rsa2048, G1Projective, HPProtocol<Bls12_381, TestHashToPrimeParameters>>::from_crs(&crs);
    ...
    c.bench_function("nonmembership_hash protocol", |be| be.iter(|| {
        let proof_transcript = RefCell::new(Transcript::new(b"nonmembership"));
        let mut verifier_channel = TranscriptVerifierChannel::new(&crs, &proof_transcript);
        let statement = Statement {
            c_e_q: commitment.clone(),
