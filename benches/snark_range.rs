...
use cpsnarks_set::{
    ...
    transcript::hash_to_prime::{TranscriptVerifierChannel, TranscriptProverChannel}
};
...
pub fn criterion_benchmark(c: &mut Criterion) {
    ...
    let crs = cpsnarks_set::protocols::membership::Protocol::<Rsa2048, G1Projective, Protocol<Bls12_381>>::setup(&params, &mut rng1, &mut rng2).unwrap().crs.crs_hash_to_prime;
    ...
    c.bench_function("snark_range protocol", |b| b.iter(|| {
        let proof_transcript = RefCell::new(Transcript::new(b"hash_to_prime"));
        let statement = Statement {
            c_e_q: commitment,
        };
